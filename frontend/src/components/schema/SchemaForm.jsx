...
const SchemaForm = ({
    ...
    return (
        <div className={`flex flex-col h-full bg-transparent overflow-hidden ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
            {/* Header / Toolbar */}
            <div className={`p-4 flex items-center justify-between border-b ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={onCancel}
                        className={`flex items-center gap-2 px-4 py-1.5 rounded text-sm font-medium transition-all ${isDark ? 'bg-slate-800 hover:bg-slate-700 border-slate-700 border' : 'bg-white hover:bg-slate-100 border-slate-200 border shadow-sm'}`}
                    >
                        <X className="w-4 h-4" />
                        CANCEL
                    </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-full">
                    ...
                    <div className="space-y-6">
                        <div className={`p-4 rounded-xl border flex flex-col h-full ${isDark ? 'bg-slate-800/40 border-slate-700' : 'bg-white border-slate-300 shadow-sm'}`}>
                            ...
                            <textarea
                                className={`flex-1 w-full bg-transparent outline-none text-[11px] leading-relaxed resize-none transition-all ${isDark ? 'text-slate-400' : 'text-slate-600'}`}
            ...
            <div className={`px-4 py-2 border-t text-[10px] flex items-center justify-between ${isDark ? 'bg-slate-900/80 border-slate-800 text-slate-500' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
                <div className="flex items-center gap-4">
